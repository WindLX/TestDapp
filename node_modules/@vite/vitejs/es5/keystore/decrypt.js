...
function decryptVersion1(keystore, pwd, selfScryptsy) {
    return new Promise(function (res, rej) {
        (0, tools_1.encryptPwd)(pwd, keystore.scryptparams, selfScryptsy).then(function (encryptP) { return res(encryptP.toString('hex') === keystore.encryptp); }).catch(function (err) {
            rej(err);
        });
    });
}
...
function decrypt(keystore, pwd, selfScryptsy) {
    var err = (0, vitejs_utils_1.checkParams)({ keystore: keystore, pwd: pwd }, ['keystore', 'pwd']);
    if (err) {
    ...
    if (!keyJson) {
        return Promise.reject({
            ...
            message: "".concat(vitejs_error_1.paramsFormat.message, " Illegal keystore.")
        });
    }
    if (keyJson.version) {
...
function _decrypt(pwd, _a, additionData, selfScryptsy) {
    ...
    var getResult = function (encryptPwd, res, rej) {
        try {
            var entropy = (0, tools_1.decipheriv)({
